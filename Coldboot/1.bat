...
for %%P in (hen_*.pkg) do (
	...
	@CertUtil -hashfile "%%P" SHA256| findstr/v : >sha.tmp
	set /p sha=<sha.tmp
	echo CUSTOM-INSTALLER_00-0000000000000000,11,[Coldboot-HEN] %%~nP,Logo and Sound on loading,,https://github.com/nikolaevich23/man/raw/master/Coldboot/%%P,!sz!,!sha!>>pkgi_tunings0.txt
)
del /q sha.tmp
